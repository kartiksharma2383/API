...
@api.post("/items")               # Defines a POST route at /items.
...
@api.get("/items", response_model = list[Item])     # Defines a GET route at /items that response or returns a list of Item objects.
...
@api.get("/items/{item_id}", response_model = Item)   # Defines a GET route at /items/{item_id} that response or returns a single Item object.
